...
sysinstall = "sudo /usr/local/sbin/pc-sysinstall"
...
def read_output(command, probar):
    call('sudo service hald stop', shell=True)
    ...
    if os.path.exists(rcconfgbsd):
        gbsd_cfg()
        call('sudo umount /media/GhostBSD', shell=True)
        sleep (2)
    # If rc.conf.desktopbsd exists run dbsd_cfg
    elif os.path.exists(rcconfdbsd):
        dbsd_cfg()
        call('sudo umount /media/DESKTOPBSD', shell=True)
        sleep (2)
    ...
    while True:
        ...
        # filer.close
        # print(bartext)
    call('sudo service hald start', shell=True)
